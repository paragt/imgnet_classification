...
class VGG16Backbone(object):
    ...
    def forward_vgg(self, inputs, training=False):
        feature_layers = []
  
        for conv in self._conv1_block:
            inputs = conv.apply(inputs)
        inputs = self._pool1.apply(inputs)
        for conv in self._conv2_block:
            inputs = conv.apply(inputs) #forward_module(conv, inputs, training=training)
        ...
        inputs = tf.contrib.layers.fully_connected(inputs, 4096) # default activation tf.nn.relu
        inputs = tf.contrib.layers.dropout(inputs) # default prob =0.5
        inputs = tf.contrib.layers.fully_connected(inputs, 4096)
        inputs = tf.contrib.layers.dropout(inputs) # default prob =0.5
        output = tf.contrib.layers.fully_connected(inputs, 1000, activation_fn=tf.nn.softmax)
