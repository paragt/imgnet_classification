...
def tower_loss(data_tensor, label_tensor, num_classes, train_mode):
    ...
    with slim.arg_scope(my_vgg_relu6_fc.vgg_arg_scope(weight_decay=args.weight_decay)):
        logits, endpoints_dict = my_vgg_relu6_fc.vgg_16_fmult(data_tensor, fmult=2, num_classes=num_classes, is_training=train_mode,dropout_keep_prob=args.dropout_keep_prob)
...
def main(args):
    ...
    #pdb.set_trace()
    model_path = '/trn_dir/models/model_multi_cl20_fmult2/model-000060' 
    ...
    with graph.as_default(), tf.device('/cpu:0'):
        ...
        vgg_trainable_variables = tf.trainable_variables(scope='vgg_16_fmult') 
        #variables_to_restore = tf.contrib.framework.get_variables_to_restore(vgg_trainable_names)
        var_load_fn = tf.contrib.framework.assign_from_checkpoint_fn(model_path,  vgg_trainable_variables)
