...
def vgg_16_fmult(inputs,
  ...
  with variable_scope.variable_scope(scope, 'vgg_16_fmult', [inputs]) as sc:
    end_points_collection = sc.original_name_scope + '_end_points'
    # Collect outputs for conv2d, fully_connected and max_pool2d.
    with arg_scope(
        [layers.conv2d, layers_lib.fully_connected, layers_lib.max_pool2d],
        outputs_collections=end_points_collection):
      net = layers_lib.repeat(
          inputs, 2, layers.conv2d, 64*fmult, [3, 3], scope='conv1')
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool1')
      net = layers_lib.repeat(net, 2, layers.conv2d, 128*fmult, [3, 3], scope='conv2')
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool2')
      net = layers_lib.repeat(net, 3, layers.conv2d, 256*fmult, [3, 3], scope='conv3')
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool3')
      net = layers_lib.repeat(net, 3, layers.conv2d, 512*fmult, [3, 3], scope='conv4')
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool4')
      net = layers_lib.repeat(net, 3, layers.conv2d, 512*fmult, [3, 3], scope='conv5')
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool5')
...
vgg_16_fmult.default_image_size = 224
...
def XORop(res,forder):

    ninp_ch = tf.shape(res)[3]
    res = tf.clip_by_value(res,0,6)
    res2 = tf.gather(res, forder, axis=3) #res[:,:,:,rndidx,:]

    fres = tf.math.multiply(res+res2, (6-res)+(6-res2)) 
    fres_clipped = tf.clip_by_value(fres, 0, 6)
    return fres_clipped

def LINEARop(res, nchannels, scope, ith):

    extra_feat = layers.conv2d(res, nchannels, [1, 1], scope=scope+'_'+str(ith+1)+'_rel')
    return extra_feat

def init_conv_corr(inp, nrepeat, noutputs, filtsz, scope, forder_in, corr_feat, corr_op):

    # inp dims: [1, fx, fy, n_input_channels, n_output_channels]
    corr_op_id = fcorr_method_ids[corr_op] 
    forder = forder_in
    with tf.variable_scope(scope):
        res = layers.conv2d(inp, noutputs, filtsz, scope=scope+'_1')
       
        #fres_clipped = tf.case([(tf.equal(corr_op_id,1) , lambda:ANDop(res, forder)),                                (tf.equal(corr_op_id,2), lambda:XORop(res,forder))], default=lambda:ANDop(res,forder))

        #fres_clipped = XORop(res,forder) 
        #fres_clipped = ANDop(res,forder) 
        fres_clipped = LINEARop(res, noutputs, scope, 1)
...
def repeat_conv_corr(inp, nrepeat, ninputs, noutputs, filtsz, scope, forder_in, corr_feat, corr_op):
    ...
    with tf.variable_scope(scope):
        for i in range(nrepeat):
            ...
            #fres_clipped = XORop(inp0, forder)
            #fres_clipped = ANDop(inp0, forder)
            fres_clipped = LINEARop(inp0, ninput_channels, scope, i)

            #fres_clipped = tf.case([(tf.equal(corr_op_id,1) , lambda:ANDop(inp0, forder)),                                    (tf.equal(corr_op_id,2), lambda:XORop(inp0,forder))], default=lambda:ANDop(inp0,forder))
...
def vgg_16_tcorr(inputs,
  ...
  with variable_scope.variable_scope(scope, 'vgg_16', [inputs]) as sc:
    ...
    with arg_scope(
      ...
      net, forder = init_conv_corr(inputs, 2, 64, [3, 3], 'conv1', forder, corr_features, corr_op)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool1')

      net, forder = repeat_conv_corr(net, 2, 64, 128, [3, 3], 'conv2', forder, corr_features, corr_op)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool2')

      net, forder = repeat_conv_corr(net, 3, 128, 256, [3, 3], 'conv3', forder, corr_features, corr_op)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool3')

      net, forder = repeat_conv_corr(net, 3, 256, 512, [3, 3], 'conv4', forder, corr_features, corr_op)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool4')

      net, _ = repeat_conv_corr(net, 3, 512, 512, [3, 3], 'conv5', forder, corr_features, corr_op)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool5')
