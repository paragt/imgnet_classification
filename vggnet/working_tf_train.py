...
def main(args):
    ...
    num_trn_examples= len(train_labels)
    ...
    with graph.as_default():
        ...
        train_dataset = train_dataset.map(_parse_function,num_parallel_calls=args.num_workers).prefetch(args.batch_size)
        train_dataset = train_dataset.map(training_preprocess,num_parallel_calls=args.num_workers).prefetch(args.batch_size)
        ...
        #val_dataset = val_dataset.map(_parse_function,num_threads=args.num_workers, output_buffer_size=args.batch_size)
        val_dataset = val_dataset.map(_parse_function,num_parallel_calls=args.num_workers).prefetch(args.batch_size)
        val_dataset = val_dataset.map(val_preprocess,num_parallel_calls=args.num_workers).prefetch(args.batch_size)
        ...
        learning_rate = tf.train.exponential_decay(0.001, step,100000, 0.96, staircase=True)
        
        fc8_optimizer = tf.train.RMSPropOptimizer(learning_rate, momentum=0.9)
        fc8_train_op = fc8_optimizer.minimize(loss, var_list=fc8_variables, global_step=step)
        ...
        opt_var_init=tf.variables_initializer(fc8_optimizer.variables())
