...
parser.add_argument('--weight_decay', default=5e-5, type=float) # google tf example uses 4e-5
...
def check_accuracy(sess,correct_prediction, image_names, labels, train_mode, dirname, labelmap_file):
    ...
    if total_examples > nbatches*bsz:
...
def main(args):
    ...
    with graph.as_default(), tf.device('/cpu:0'):
        ...
        nu_epoch = tf.cast(exm_per_epoch*1.0/(args.batch_size*args.num_gpus), tf.int64) # num_update_per_epoch
        #init_lr = 0.01*(0.94**50) #0.00075
        init_lr = 0.01*(0.94**41) #0.00075

        # piecewise
        #boundaries = [2*nu_epoch, 4*nu_epoch, 6*nu_epoch, 8*nu_epoch, 10*nu_epoch, 12*nu_epoch, 14*nu_epoch, 16*nu_epoch, 18*nu_epoch, 20*nu_epoch]
        #boundaries = [10000, 20000, 30000, 40000, 60000, 70000 , 80000, 90000, 100000, 110000]
        #values=[init_lr, init_lr*0.9, init_lr*0.8, init_lr*0.7, init_lr*0.6, init_lr*0.5, init_lr*(0.4), init_lr*(0.3), init_lr*(0.2), init_lr*(0.1), init_lr*(0.1)]

        #learning_rate_piecewise = tf.train.piecewise_constant(step, boundaries, values)


        # linear decay
        #TMAX = tf.cast(nu_epoch*(args.num_epochs2+1), dtype=tf.float32)
        #learning_rate_linear = lrmax - (lrmax-lrmin)*(tf.cast(step,dtype=tf.float32)/TMAX)      
        
        # linear epoch-wise
        #TMAX = tf.cast(args.num_epochs2+1, dtype=tf.float32)
        #epc = tf.div(step,nu_epoch)
        #learning_rate_linear = lrmax - (lrmax-lrmin)*(tf.cast(epc,dtype=tf.float32)/TMAX)      

        # skeleton decay
        #decay_rate = ((lrmax/lrmin)-1)/TMAX #5e-5
        #epc = tf.div(step,nu_epoch)
        #learning_rate_decay= (lrmax /(1. + (epc)*decay_rate))
        
        # exponential decay
        decay_step_exp = nu_epoch*1
        learning_rate_exp = tf.train.exponential_decay(init_lr, step, decay_step_exp, 0.96, staircase=True) #0.97
        ...
        #full_optimizer = MyAdamW(weight_decay=0.001, learning_rate=learning_rate,beta1=0.9,beta2=0.99, epsilon=0.1)
        ...
        variables_to_restore = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='vgg_16')
        var_load_fn = tf.contrib.framework.assign_from_checkpoint_fn('/trn_dir/models/model_vgg_preprocess/model-000080', variables_to_restore)
    ...
    with tf.Session(graph=graph) as sess:
        ...
        var_load_fn(sess)
        #new_saver = tf.train.import_meta_graph('/trn_dir/models/model1_imgnet/model-000030.meta')
        #new_saver.restore(sess, '/trn_dir/models/model1_imgnet/model-000030')
        ...
        for epoch in range(args.num_epochs2):
            ...
            #rand_idx = np.random.choice(total_examples,total_examples,replace=True)
