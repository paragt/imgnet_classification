...
def main(args):
    ...
    with graph.as_default():
        ...
        with slim.arg_scope(vgg_relu6.vgg_arg_scope(weight_decay=args.weight_decay)):
            logits, endpoints_dict = vgg_relu6.vgg_16(images, num_classes=num_classes, is_training=train_mode,dropout_keep_prob=args.dropout_keep_prob)

        tf.add_to_collection("logits",logits)

        #loss = tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(labels=labels, logits=logits))
        # replace following by above
