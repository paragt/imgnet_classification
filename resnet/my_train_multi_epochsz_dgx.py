...
def main(args):
    ...
    with graph.as_default(), tf.device('/cpu:0'):
        ...
        #full_optimizer = tf.train.MomentumOptimizer(learning_rate, momentum=0.9,use_nesterov=False)
        ...
        minimize_op = full_optimizer.apply_gradients(avg_grads, global_step=step)
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for epoch in range(args.num_epochs2):
            ...
            while not is_eoe:
                ...
                sess.run(train_op, feed_dict={image_names: batch_filenames, labels: batch_labels, train_mode: True})#, learning_rate: current_lr})
                ...
                if trn_iteration%100 == 0:
                    msummary = sess.run(merged_summary, feed_dict={image_names: batch_filenames, labels: batch_labels, train_mode: True})#, learning_rate: current_lr})   
                    train_writer.add_summary(msummary, trn_iteration)
                if trn_iteration%100 == 0:
                    tower_losses,batch_accuracy,lr = sess.run([loss_array,accuracy, learning_rate], feed_dict={image_names: batch_filenames, labels: batch_labels, train_mode: True})#, learning_rate: current_lr})
            ...
            if epoch%2==0:
                ...
                acc_diff = val_acc - prev_acc
                print('prev acc : %f' % prev_acc)
                print('acc diff: %f' % acc_diff)
                if acc_diff<0.005:
                    wait_count += 1
                else:
                    wait_count = 0
...
'''
                ...
                if len(recent_acc)>4:
                    recent_acc.pop(0)
                acc_diff = np.abs(np.diff(recent_acc))
                print(recent_acc)
                if len(recent_acc)>1:
                ...
                if len(recent_acc)<4:
                    ...
                    change_acc = recent_acc[-1]-recent_acc[0]
                print(mean_acc_diff)
                print(change_acc)
                if (mean_acc_diff<0.005) or (change_acc<0.005):
                    print('reducing lr')
